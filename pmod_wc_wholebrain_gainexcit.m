...
for ei = 1 : length(Ies)
  ...
  for igain = 1 : length(Gains)
    for iexc = 1 : length(Excit)
      %
      if ~exist(sprintf(['~/pmod/proc/' 'pmod_wc_wholebrain_gainexcit_ei%d_gain%d_excit%d_v%d_processing.txt'],ei,igain,iexc,v))
        system(['touch ' '~/pmod/proc/' sprintf('pmod_wc_wholebrain_gainexcit_ei%d_gain%d_excit%d_v%d_processing.txt',ei,igain,iexc,v)]);
      else
        continue
      end
      tic
...
%         out.dfa_env(tr,:)   = tmp.exp;
      ...
      save(sprintf('~/pmod/proc/pmod_wc_wholebrain_gainexcit_ei%d_gain%d_excit%d_v%d.mat',ei,igain,iexc,v),'out')
