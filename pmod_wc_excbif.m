...
% Ies       = -0.2:0.05:1.5;
% iG        = 0;
% inh_gain  = 0.9:0.02:1.1;
% exc_gain  = 0.9:0.02:1.1;
% Iis       = -4.5:0.02:-3.5;
% -------------------------------
% VERSION 3
% -------------------------------
...
% Ies       = -0.5:0.075:1.5;
% iG        = 0;
% inh_gain  = 0.95:0.01:1.05;
% exc_gain  = 0.95:0.01:1.05;
% Iis       = -4.4:0.04:-3.6;
% -------------------------------
% VERSION 4
% -------------------------------
...
for i_inh_gain = 1 : length(inh_gain)
  for i_exc_gain = 1 : length(exc_gain)
    for j = 1 : length(Iis)
      for Q = 1  % conditions (equivalent gain change, daE<daI, daE>daI)
        
        for ip = 1 : length(SIG)

          if ~exist(['~/pmod/proc/' sprintf('pmod_wc_excbif_q%d_ii%d_p%d_inhgain%d_excgain%d_v%d.txt',Q,j,ip,i_inh_gain,i_exc_gain,v)])
            system(['touch ' '~/pmod/proc/' sprintf('pmod_wc_excbif_q%d_ii%d_p%d_inhgain%d_excgain%d_v%d.txt',Q,j,ip,i_inh_gain,i_exc_gain,v)]);
          else
            continue
          end
...
%           for ig = 1: length(GAIN)
            for k = 1:numIes
              ...
              Fe = @(x) 1./(1 + exp((-x+InpG)/aE) );
              Fi = @(x) 1./(1 + exp((-x+InpG)/aI) );
...
%                 fprintf('Computing Gain%d, Ie%d, trial%d ...\n',ig,k,trial)
                fprintf('Computing I-Gain%d, E-Gain%d, Inh%d, trial%d ...\n',i_inh_gain,i_exc_gain,j,trial)
                ...
                ampcorr(k,trial,igain) = corr(Ampl(:,1),Ampl(:,2));
              end
              end
            end
            
            fprintf('Saving...\n');
            save(sprintf('~/pmod/proc/pmod_wc_excbif_q%d_ii%d_p%d_inhgain%d_excgain%d_v%d.mat',Q,j,ip,i_inh_gain,i_exc_gain,v),'ampcorr','-v7.3')
            
            try
              load(sprintf('~/pmod/proc/pmod_wc_excbif_q%d_ii%d_p%d_inhgain%d_excgain%d_v%d.mat',Q,j,ip,i_inh_gain,i_exc_gain,v));
            catch me
              pause(randi(10))
              save(sprintf('~/pmod/proc/pmod_wc_excbif_q%d_ii%d_p%d_inhgain%d_excgain%d_v%d.mat',Q,j,ip,i_inh_gain,i_exc_gain,v),'ampcorr','-v7.3')
            end
...
%%
clear Rc RcAmpl tmp_rc
v = 4;
% tmp_rc = zeros(20,length(GAIN),length(SIG));
iii = 1:5;
    ...
    for j = 1 : length(Iis)
      for Q = 1  % conditions (equivalent gain change, daE<daI, daE>daI)
        ...
        for ip = 1 : length(SIG)
            ...
            load(sprintf('~/pmod/proc/pmod_wc_excbif_q%d_ii%d_p%d_inhgain%d_excgain%d_v%d.mat',Q,j,ip,i_inh_gain,i_exc_gain,v));
            tmp_rc(i_inh_gain,i_exc_gain,j,:,:)    = mean(ampcorr,2);
...
% imagesc(squeeze(tmp_rc(i_inh_gain,i_exc_gain,:,:)),[0 0.2])
imagesc(squeeze(tc(50,:,:)),[0.5 1])
...
for iinh = 1 : 11
  ...
  for ii = 1 : 11
    rc= (tmp_rc(:,:,:,1,iinh,ii)-repmat(tmp_rc(:,find(GAIN==0),:,1,iinh,ii),[1 length(GAIN) 1]));%.*(tmp_rc(:,:,:,ii)>0.5 & tmp_rc(:,:,:,ii)<0.8);
    ...
    set(gca,'xtick',1:9:length(Ies),'xticklabel',Ies(1:9:length(Ies)))
    set(gca,'ytick',1:2:length(GAIN),'yticklabel',GAIN(1:2:length(GAIN)))
    set(gca,'tickdir','out')
    
    title(sprintf('Exc. %.2f',exc_gain(ii)))
...
for isig = 1 : length(SIG)
  ...
  imagesc(d',[-0.6 0.6]); axis square
  d = squeeze(Rc_all(:,2,:,isig)-Rc_all(:,1,:,isig));
  
  d=d.*(d>0.01 & squeeze(Rc_all(:,1,:,isig))<0.75 & squeeze(Rc_all(:,2,:,isig))>0.5);
  %   d=d.*(d<0.03 & squeeze(Rc_all(:,1,:,isig))>-0.2 & squeeze(Rc_all(:,2,:,isig))>-0.2);
  
  subplot(length(SIG),2,(isig-1)*2+2)
  imagesc(d',[-0.5 0.5]); axis square
  clear d
