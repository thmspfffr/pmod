...
%-------------------------------------------------------------------------
% VERSION 1: Same as v3 in *whole_brain_final.m, but modyfing noise
% %-------------------------------------------------------------------------
v           = 1;
Ies         = -4:0.025:-1;
Iis         = -5:0.025:-2;
    ...
    for inoise = 1 : length(noise_mod)
      for iies = 1 : length(Ies)
        for iiis = 1: length(Iis)
          ...
          if ~exist(sprintf('~/pmod/proc/noise/v%d/',v))
            mkdir(sprintf(['~/pmod/proc/noise/' 'v%d'],v))
          end
          ...
          fn = sprintf('pmod_wc_wholebrain_final_Ie%d_Ii%d_noise%d_G%d_gain%d_v%d',iies,iiis,inoise,iG,igain,v);
