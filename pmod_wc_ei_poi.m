...
for iies = 1: length(Ies)
  for ei = 1 : length(EIs)
    for igain = 1 : length(Gains)
      ...
      fn = sprintf('pmod_wc_ei_poi_Ie%d_ei%d_gain%d_v%d',iies,ei,igain,v);
...
for iies = 1: length(Ies)
  for iiis = 1: length(Iis)
    for ei = 1 : length(EIs)
      ...
      if ~exist(sprintf('~/pmod/proc/ei/pmod_wc_ei_Ie%d_Ii%d_ei%d_v%d.mat',iies,iiis,ei,v))
        if exist(sprintf('~/pmod/proc/ei/pmod_wc_ei_Ie%d_Ii%d_ei%d_v%d_processing.txt',iies,iiis,ei,v))
          delete(sprintf('~/pmod/proc/ei/pmod_wc_ei_Ie%d_Ii%d_ei%d_v%d_processing.txt',iies,iiis,ei,v))
          warning('Deleting')
        else
          continue
        end
