...
fc_tvr =  squeeze(nanmean(cleandat(:,:,:,1,2,6),3))- squeeze(nanmean(cleandat(:,:,:,1,1,6),3));
...
for idIE = 1 : length(dIE)
  for idII = 1 : length(dII)
    ...
    if ~exist(sprintf(['~/pmod/proc/' 'pmod_WC_wholebrain_task_idIE%d_idII%d_processing.txt'],idIE,idII))
      system(['touch ' '~/pmod/proc/' sprintf('pmod_WC_wholebrain_task_idIE%d_idII%d_processing.txt',idIE,idII)]);
    else
      continue
    end
    % input due to task
...
%         Epsilon{2}(i,tr) = lags(ii)*resol;
    ...
    out = struct('dfa',dfa,'FC',FC,'dIe',dIe,'dIi',dIi);
...
for idIE = 1 : length(dIE)
  for idII = 1 : length(dII)
    ...
    try
    load(sprintf('~/pmod/proc/pmod_WC_wholebrain_taskvsrest_idIE%d_idII%d.mat',idIE,idII),'out')
    
    FC_all(:,:,:,idIE,idII) = out.FC;
%     dfa_all(:,:,idIE,idII)  = out.dfa;
    catch me
      FC_all(:,:,:,idIE,idII) = nan(90,90,2);
    end
    ...
    fc_diff = tp_match_aal(pars,FC_all(:,:,2,idIE,idII)-FC_all(:,:,1,idIE,idII));
    
    r(idIE,idII)=corr(fc_diff(mask),fc_tvr(mask))
