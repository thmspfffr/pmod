        ...
        for iasym = 1 : length(ASYM)
          ...
          for iexc = 1:numIes
            ...
            for trial = 1:nTrials
              ...
              d = tp_dfa(Ampl(:,1),[3 30],100,0.5,20);
              
              dfa(iexc,1) = dfa(iexc,1) + d.exp/nTrials;
              
              d = tp_dfa(Ampl(:,2),[3 30],100,0.5,20);
          ...
          fprintf('Saving...\n');
          save(sprintf('~/pmod/proc/pmod_wc_orig_gain%d_noise%d_inh%d_asym%d_v%d.mat',igain,inoise,iinh,iasym,v),'Rc','RcAmpl','dfa','-v7.3')
          
          try
            load(sprintf('~/pmod/proc/pmod_wc_orig_gain%d_noise%d_inh%d_asym%d_v%d.mat',igain,inoise,iinh,iasym,v))
          catch me
            pause(randi(10))
            save(sprintf('~/pmod/proc/pmod_wc_orig_gain%d_noise%d_inh%d_asym%d_v%d.mat',igain,inoise,iinh,iasym,v),'Rc','RcAmpl','dfa','-v7.3')
          end
    ...
    for iinh = 1 : length(Iis)
      for inoise = 1 : length(SIGMA)
        for iasym = 1 : length(ASYM)
          ...
          load(sprintf('~/pmod/proc/pmod_wc_orig_gain%d_noise%d_inh%d_asym%d_v%d.mat',igain,iinh,inoise,iasym,v))
...
subplot(3,4,4)
imagesc(alll(:,2:end,5)'-alll(:,2:end,1)',[-0.2 0.2])
