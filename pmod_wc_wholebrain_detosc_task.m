%% pmod_wc_wholebrain_detosc_task
% Determine oscillatory regime for task
%-------------------------------------------------------------------------
...
for isubj = 1:size(idx_rest.exc,2)
  for igain = find(Gains==0)%1:length(Gains)
    for iG = find(Gg==1.15)%1:length(Gg)
      ...
      if ~exist(sprintf('~/pmod/proc/detosc//task/v%d/',v))
        mkdir(sprintf(['~/pmod/proc/detosc//task/' 'v%d'],v))
      end
      
      outdir = sprintf(['~/pmod/proc/detosc//task/' 'v%d/'],v);
      
      fn = sprintf('pmod_wc_wholebrain_detosc_task_isubj%d_v%d',isubj,v);
      ...
      for itask_E = 1 : length(task_Es)
        for itask_I = 1 : length(task_Is)
          ...
          for tr=1:nTrials
            ...
            for i=1:N
              [out.osc1(i,itask_E,itask_I,tr) ] = tp_detect_osc(R(:,i));
            end
